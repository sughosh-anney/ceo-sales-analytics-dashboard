...
Verified 2026-09 against this file directly:
  ...
  - 'Summary-ADFL Conso 2025' sheet: same per-entity 8-column-per-month
    block layout (7 entities + Total), now with a 5th month block
    (Aug, cols AI:AP) appended after Jul (cols AA:AH) -- Apr/May/Jun/Jul
    column positions are UNCHANGED from the July file. The August block's
    2nd entity is labeled "Australia" directly (the July file's blocks
    all say "ADFIL" for the same entity) -- both map to the same
...
# Source workbook locations are supplied by the environment -- nothing
# machine-specific is hard-coded here.
MIS_AUG_PATH = Path(os.environ.get("CONSO_MIS_WORKBOOK_AUG", ""))

_xlsm_candidates = sorted(BASE.glob("CEO Standalone and Conso Dashboard Data_V*.xlsm"))
if _xlsm_candidates:
    C.XLSM_PATH = _xlsm_candidates[-1]
...
def main():
    ...
    print(f"Writing into {OUT_PATH.name}'s RawData_MIS tab ...")
    ...
    ws["A2"] = ("Sourced from the board MIS report (ADF_Conso MIS_August 26- without tariff duty refund-8.9.2026-v1.xlsx, "
                "Summary-ADFL Conso 2025 sheet) -- refreshed 2026-09 to a 5-month (Apr-Aug) YTD basis, replacing the "
                "earlier 4-month (Apr-Jul) figures. NOT raw-transaction-driven -- a small pasted reference table, same "
                "architecture the original Standalone workbook itself uses for its own Entity P&L data. Note the source "
                "file's own name flags it excludes a tariff duty refund -- confirm with Finance whether that matters for "
    ...
    pbt_cur = val(23, "S")
    pat_cur = val(26, "S")
    print(f"True elim-adjusted Group (5M FY27): Sales={sales_cur:.1f} EBITDA={ebitda_cur:.1f} PBT={pbt_cur:.1f} PAT={pat_cur:.1f} (PY Sales={sales_py:.1f})")
